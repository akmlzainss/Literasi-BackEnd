                                    ...
                                    <div class="modal-body">
                                        <div class="mb-3">
                                            ...
                                            <select name="status" id="status{{ $video->id }}" class="form-select"
                                                ...
                                                <option value="menunggu" {{ $video->status === 'menunggu' ? 'selected' : '' }}>
                                                    Menunggu</option>
                                                <option value="disetujui" {{ $video->status === 'disetujui' ? 'selected' : '' }}>
                                                    Disetujui</option>
                                                <option value="ditolak" {{ $video->status === 'ditolak' ? 'selected' : '' }}>
                                                    Ditolak</option>
                                            </select>
                                        </div>
                                    ...
                                    <div class="modal-body">
                                        ...
                                        <div class="mb-3">
                                            <label for="deskripsi{{ $video->id }}" class="form-label">Deskripsi
                                                Video</label>
                                            <textarea name="deskripsi" id="deskripsi{{ $video->id }}" class="form-control" rows="3"
                                                required>{{ $video->deskripsi }}</textarea>
                                        </div>
                                        <div class="row">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.modal').on('shown.bs.modal', function() {
                ...
                $(this).find('.kategori-edit-select2').each(function() {
                    ...
                    initializeSelect2('#' + id, '{{ route('admin.search.kategori') }}', 'Pilih Kategori',
            ...
            $('#filterForm').on('submit', function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        const newTotal = $(response).find('.card-header-custom .d-flex span').text();
        ...
        `;
            ...
            window.toggleAlasan = function(status, alasanContainerId) {
                document.getElementById(alasanContainerId).style.display = status === 'ditolak' ? 'block' : 'none';
            }

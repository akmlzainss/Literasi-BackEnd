...
@section('content')
    ...
    <div class="page-header">
        ...
        @php
            // Ambil format Y-m dari variabel $selectedMonth
            $monthFilter = \Carbon\Carbon::parse($selectedMonth)->format('Y-m');
        @endphp

        <a href="{{ route('admin.penghargaan.index', ['month' => $monthFilter]) }}" class="btn btn-secondary">
            ...
            <div class="card-body-custom">
                @if ($topItems->isEmpty())
                    <div class="alert alert-warning">
                        Tidak ada {{ $preSelectedType ?? 'artikel' }} tersedia untuk bulan ini. Silakan unggah konten
                        terlebih dahulu atau pilih bulan lain.
                        <a href="{{ route('admin.penghargaan.index') . '?month=' . $selectedMonth }}"
                            class="alert-link">Kembali ke daftar</a>.
                    </div>
                @else
                    <form action="{{ route('admin.penghargaan.store') }}" method="POST">
                        ...
                        <div class="mb-3">
                            <label for="id_item" class="form-label">Pilih {{ ucfirst($preSelectedType ?? 'Artikel') }} (Top
                                5 {{ $preSelectedType ?? 'artikel' === 'artikel' ? 'Rating' : 'Like' }})</label>
                            <select name="id_item" class="form-control" id="id_item"
                                {{ isset($preSelectedId) ? 'disabled' : '' }} required>
                                <option value="">-- Pilih {{ ucfirst($preSelectedType ?? 'Artikel') }} --</option>
                                @foreach ($topItems as $item)
                                    <option value="{{ $item->id }}"
                                        {{ $preSelectedId == $item->id ? 'selected' : '' }}
                                        data-judul="{{ $item->judul }}" data-nama="{{ $item->siswa_nama ?? 'Unknown' }}"
                                        ...
                                        data-rating="{{ $item->rating ?? 0 }}" data-type="{{ $item->type }}">
                                        {{ $item->judul }} ({{ $item->type === 'artikel' ? 'Rating' : 'Like' }}:
                        ...
                        {{-- Info Preview --}}
                        <div class="mb-3" id="itemInfo"
                            style="display: {{ isset($preSelectedId) ? 'block' : 'none' }};">
                                ...
                                <div class="row g-0">
                                    ...
                                    <div class="col-md-8">
                                        <div class="card-body">
                                            ...
                                            <p class="card-text mb-1"><strong>Penulis:</strong> <span
                                                    id="itemPenulis"></span></p>
                                            <p class="card-text mb-1"><strong>Kelas:</strong> <span id="itemKelas"></span>
                                            </p>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" name="bulan_tahun" id="bulan_tahun" class="form-control"
                                value="{{ old('bulan_tahun', \Carbon\Carbon::parse($selectedMonth)->format('Y-m-d')) }}"
                                required>
                            @error('bulan_tahun')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="mb-3">
                            ...
                            <select name="jenis" id="jenis" class="form-control" required>
                                ...
                                <option value="bulanan" {{ old('jenis', 'bulanan') == 'bulanan' ? 'selected' : '' }}>
                                    Bulanan</option>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                const updatePreview = (selected) => {
                    if (selected && selected.value) {
                        ...
                        itemLabel.innerHTML = type === 'artikel' ? '<strong>Rating:</strong>' :
                            '<strong>Like:</strong>';
                ...
                if (itemSelect) {
                    ...
                    @if (isset($preSelectedId))
                        setTimeout(() => {
                            const selectedOption = itemSelect.querySelector(
                                'option[value="{{ $preSelectedId }}"]');
                            if (selectedOption) {

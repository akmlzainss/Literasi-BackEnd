        ...
        <div class="card-body-custom">
            <div class="award-card fade-in">
                <div class="award-header"
                    style="background: linear-gradient(135deg, {{ $penghargaan->jenis == 'bulanan' ? '#f59e0b, #d97706' : '#8b5cf6, #7c3aed' }});">
                    <h5 class="award-title">{{ $penghargaan->deskripsi_penghargaan }}</h5>
                    <span
                        class="award-status {{ \Carbon\Carbon::parse($penghargaan->bulan_tahun)->isPast() ? 'status-inactive' : 'status-active' }}">
                ...
                <div class="award-content">
                    ...
                    <p><strong>Bulan/Tahun:</strong>
                        {{ \Carbon\Carbon::parse($penghargaan->bulan_tahun)->translatedFormat('F Y') }}</p>
                    <p><strong>Siswa:</strong>
                        {{ $penghargaan->siswa ? $penghargaan->siswa->nama : 'Siswa tidak ditemukan' }}</p>
                    <p><strong>Artikel:</strong>
                        {{ $penghargaan->artikel ? $penghargaan->artikel->judul : 'Tidak ada artikel' }}</p>
                    <p><strong>Diberikan oleh:</strong>
                        {{ $penghargaan->admin ? $penghargaan->admin->nama : 'Admin tidak ditemukan' }}</p>
                    <p><strong>Tanggal Dibuat:</strong>
                        {{ \Carbon\Carbon::parse($penghargaan->dibuat_pada)->translatedFormat('d F Y H:i') }}</p>
                    <div class="award-actions">
                        <button class="btn-action-card btn-edit-card" data-id="{{ $penghargaan->id }}"
                            data-bs-toggle="modal" data-bs-target="#modalEditPenghargaan">

...
@section('content')
    ...
    <div class="page-header">
        ...
        <p class="page-subtitle">
            Bulan: <strong>{{ \Carbon\Carbon::parse($selectedMonth)->translatedFormat('F Y') }}</strong> |
            {{ ucfirst($preSelectedType) }}
        </p>
                ...
                <form action="{{ route('admin.penghargaan.store') }}" method="POST">
                        ...
                        <select name="id_item" id="id_item" class="form-control"
                            ...
                            @foreach ($topItems as $item)
                                <option value="{{ $item->id }}" {{ $preSelectedId == $item->id ? 'selected' : '' }}
                                    data-judul="{{ $item->judul }}" data-nama="{{ $item->nama }}"
                                    data-kelas="{{ $item->kelas }}" {{-- PERBAIKAN 1: Tambahkan data-item-type ke data-foto, dan gunakan default jika kosong --}}
                                    data-foto="{{ $item->foto_url ?? ($preSelectedType === 'artikel' ? asset('images/default-buku.jpg') : asset('images/default-kamera.jpg')) }}"
                                    data-rating="{{ $item->rating }}" data-item-type="{{ $preSelectedType }}">
                        ...
                        <div class="card shadow-sm border-0" style="max-width: 500px;">
                            <div class="row g-0">
                                <div class="col-md-4">
                                    ...
                                    @php
                                        $defaultImg =
                                            $preSelectedType === 'artikel'
                                                ? asset('images/default-buku.jpg')
                                                : asset('images/default-kamera.jpg');
                                        $initialFoto = '';
                                        if (isset($preSelectedId)) {
                                            $selectedItem = $topItems->firstWhere('id', $preSelectedId);
                                            $initialFoto = $selectedItem
                                                ? $selectedItem->foto_url ?? $defaultImg
                                                : $defaultImg;
                                        } else {
                                            $initialFoto = $defaultImg;
                                        }
                                    @endphp
                                    <img id="itemFoto" src="{{ $initialFoto }}" class="img-fluid rounded-start"
                                        alt="Item Image">
                                </div>
                                <div class="col-md-8">
                                    <div class="card-body">
                                        <h5 class="card-title mb-1" id="itemJudul"></h5>
                                        <p class="card-text mb-1"><strong>Penulis:</strong> <span id="itemPenulis"></span>
                                        </p>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="date" name="bulan_tahun" class="form-control"
                            value="{{ old('bulan_tahun', \Carbon\Carbon::parse($selectedMonth)->format('Y-m-01')) }}"
                            required>
                        @error('bulan_tahun')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3">
                        ...
                        <select name="jenis" class="form-control" required>
                            <option value="bulanan" {{ old('jenis', 'bulanan') == 'bulanan' ? 'selected' : '' }}>Bulanan
                            </option>
            ...
            const updatePreview = (option) => {
                ...
                if (option.value) {
                    ...
                    document.getElementById('itemLabel').innerHTML =
                        `<strong>${isArtikel ? 'Rating' : 'Like'}:</strong>`;
                    document.getElementById('itemRating').textContent = isArtikel ? rating.toFixed(1) : Math
                        .round(rating);
